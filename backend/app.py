...
def health():
    ...
    for tx in txs:
        # Standard tuple unpacking or row index referencing
...
def user_info():
    ...
    conn = db_layer.get_connection()
    # Using mysql connector DictCursor or similar if available, else standard sqlite cursor row_factory
    # We will fetch manually based on custom row index mapping to support standard systems.
    cursor = conn.cursor()
